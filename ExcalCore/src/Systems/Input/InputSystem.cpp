...
glm::vec2 InputSystem::GetAxis2D(const std::string &axis_id) {
    try {
        ...
        return InputSystem::GetAxis2D(bind->up, bind->down, bind->left, bind->right);
    } catch (const std::bad_cast& e) {
        Debug::LogWarning("Binding '{}' is not of type 'Button'", axis_id);
...
glm::vec3 InputSystem::GetAxis3D(const std::string &axis_id) {
    try {
        ...
        return InputSystem::GetAxis3D(bind->up, bind->down, bind->left, bind->right, bind->up, bind->down);
    } catch (const std::bad_cast& e) {
        Debug::LogWarning("Binding '{}' is not of type 'Button'", axis_id);
...
float InputSystem::GetAxis(const KeyCode positive, const KeyCode negative) const {
    float ret = 0.0f;
...
glm::vec2 InputSystem::GetAxis2D(const KeyCode up, const KeyCode down, const KeyCode left, const KeyCode right) const {
...
glm::vec3 InputSystem::GetAxis3D(const KeyCode up, const KeyCode down, const KeyCode left, const KeyCode right,
    const KeyCode front, const KeyCode back) const {
...
void InputSystem::BindButton(const std::string &button_id, KeyCode key) {
    _binding_map.insert_or_assign(button_id, std::make_unique<InputBindingButton>(key));
}

void InputSystem::BindAxis(const std::string &axis_id, KeyCode positive, KeyCode negative) {
    _binding_map.insert_or_assign(axis_id, std::make_unique<InputBindingAxis>(positive, negative));
}

void InputSystem::BindAxis2D(const std::string &axis_id, KeyCode up, KeyCode down, KeyCode left, KeyCode right) {
    _binding_map.insert_or_assign(axis_id, std::make_unique<InputBinding2D>(up, down, left, right));
}

void InputSystem::BindAxis3D(const std::string &axis_id, KeyCode up, KeyCode down, KeyCode left,
    KeyCode right, KeyCode front, KeyCode back) {
    _binding_map.insert_or_assign(axis_id, std::make_unique<InputBinding3D>(up, down, left, right, front, back));
}
