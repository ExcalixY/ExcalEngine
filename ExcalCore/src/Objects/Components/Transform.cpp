...
void Transform::SetPosition(const glm::vec3 &pos) { _pos = pos; }

void Transform::SetRotation(const glm::quat &rot) { _rot = rot; }

void Transform::SetScale(const glm::vec3 &scale) { _scale = scale; }

void Transform::Move(const glm::vec3 &move_vec) { _pos += move_vec; }

void Transform::Move(float x, float y, float z) { _pos += glm::vec3(x, y, z); }
...
void Transform::Rotate(const float euler_x, const float euler_y,
                       const float euler_z) {
