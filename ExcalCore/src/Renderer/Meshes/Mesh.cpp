...
Mesh::Mesh(std::string_view mesh_file_path) {
  ...
  if (!input.is_open()) {
    Debug::LogError("Could not find Mesh File in Path: {}",
                    mesh_file_path.data());
    return;
  }
...
void Mesh::BufferData() const {
  glBindBuffer(GL_ARRAY_BUFFER, _VBO);
  glBufferData(GL_ARRAY_BUFFER, _data.vertices.size() * sizeof(Vertex),
               _data.vertices.data(), GL_STATIC_DRAW);

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _EBO);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               _data.indices.size() * sizeof(unsigned int),
               _data.indices.data(), GL_STATIC_DRAW);
