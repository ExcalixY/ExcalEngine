...
Material::Material(const std::string &path_ver, const std::string &path_frag)
    : _shader_vert(path_ver, Shader::VERTEX),
      _shader_frag(path_frag, Shader::FRAGMENT) {
...
void Material::SetUniformMat4(const std::string &name,
                              const glm::mat4 &mat) const {
  glProgramUniformMatrix4fv(_program, GetUniformLocation(name), 1, GL_FALSE,
                            glm::value_ptr(mat));
}

void Material::SetUniformMat3(const std::string &name,
                              const glm::mat3 &mat) const {
  glProgramUniformMatrix3fv(_program, GetUniformLocation(name), 1, GL_FALSE,
                            glm::value_ptr(mat));
}

auto Material::SetUniformVec4(const std::string &name,
                              const glm::vec4 &vec) const -> void {
  glProgramUniform4fv(_program, GetUniformLocation(name), 1, &vec[0]);
}

void Material::SetUniformVec3(const std::string &name,
                              const glm::vec3 &vec) const {
  glProgramUniform3fv(_program, GetUniformLocation(name), 1, &vec[0]);
}

void Material::SetUniformVec2(const std::string &name,
